        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                #     world.hud.toggle_info()
                    ...
                    world.player.enable_zmq_physics("tcp://10.196.16.114:5555", "tcp://10.196.16.114:5556")
...
def game_loop(args, sock):
    ...
    # Load the data
    import pandas as pd
    df = pd.read_csv("savedData_Mugello_3D_F142MFL.csv")
    ...
    try:
        ...
        for _, row in df.iterrows():
            debug.draw_point(
                carla.Location(row["x_veh"], -row["y_veh"], row["z_veh"]),
                life_time=0
            )
            #debug.draw_point(
            #    carla.Location(row["xL"], -row["yL"], row["altL"]),
            #    life_time=0
            #)
...
def main():
    ...
    argparser.add_argument(
        ...
        metavar='Uip',
        default='127.0.0.1',
        help='IP of the UDP server (default:127.0.0.1)')
    argparser.add_argument(
        '-Up','--UDPport',
        metavar='P',
        default=5006,
